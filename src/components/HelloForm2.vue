  ...
  methods: {
    ...
    save(record) {
      ...
      if (target) {
        ...
        baseFetch({
          method: "PUT",
          url: `/v1/operatings/${this.record.id}`,
          data: {
            a,
          },
        })
    ...
    isConsumeRatio(text, record) {
      text = this.toNum(text);
      return (text > 8 && record?.income > 500) || (text > 15 && record?.consume > 50);
    },
    ...
    unSatisfies(record) {
      const { income, consume_ratio, cost_ratio, settlea_30, platform } = record;
      let list = [];
      console.log(record);
