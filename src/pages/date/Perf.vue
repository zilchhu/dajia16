...
export default {
  ...
  methods: {
    ...
    showChartModal(record, col) {
      let perf_realshop = this.perfs.filter((v) => v.real_shop == record.real_shop)
        ...
        series: [
          {
            name: col,
            type: "line",
            data: this.perf_dates.map((d) => perf_realshop.find((v) => v.date == d)[col]),
          },
        ],
      }
    ...
    transformTable() {
      return this.perfs.map((row) =>
        this.perf_columns.reduce((p, c) => ({ ...p, [c.title]: row[c.dataIndex] }), {})
      )
    },
    exportTable() {
      this.exporting = true
