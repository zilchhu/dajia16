<template lang="pug">
.tools-collect-coupon
  .title 收藏有礼
  .task-table
    .task-table-header
      a-button(size="small") 新增任务
    .task-table-body
      a-table()
  ...
  export default {
    ...
    data() {
      return {
        ...
        loading: false,
        tableName: "",
        templateUrl: "http://192.168.3.3:9007/收藏领券.xlsx",
      ...
      columns() {
        return [
          ...
          },
          {
            ...
            width: 90,
            slots: { filterDropdown: "filterDropdown" },
            onFilter: (value, record) => record.使用门槛 == value,
          },
          {
            title: "优惠券金额",
            dataIndex: "优惠券金额",
            width: 90,
            slots: { filterDropdown: "filterDropdown" },
            onFilter: (value, record) => record.优惠券金额 == value,
          },
          {
            title: "使用有效期",
            dataIndex: "使用有效期",
    ...
    methods: {
      ...
      run() {
        ...
        app.run("ws://", "update-collect-coupon-acts", {
    ...
    mounted() {
      ...
      app.on("update-collect-coupon-acts-res", (state) => {
        console.log(state);
